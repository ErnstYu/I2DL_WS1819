...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        self.conv = nn.Conv2d(channels, num_filters, kernel_size,
                              stride=stride_conv, padding=(kernel_size-1)//2)
        self.conv.weight.data.mul_(weight_scale)
        
        self.pool = nn.MaxPool2d(pool, stride=stride_pool)
        pool_height = (height - pool)//stride_pool + 1
        pool_width  = (width  - pool)//stride_pool + 1
    ...
    def forward(self, x):
        ...
        sizes = x.size()[1:]
        x = x.view(-1, sizes[0] * sizes[1] * sizes[2])
        x = self.fc1(x)
