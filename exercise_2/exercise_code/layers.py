...
def affine_forward(x, w, b):
    ...
    #############################################################################
    xRows = np.reshape(x, (x.shape[0], -1))
    out = np.dot(xRows, w) + b
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, w, b)
...
def batchnorm_backward(dout, cache):
    ...
    #############################################################################
    N = dout.shape[0]
    _, x_norm, beta, gamma, x_minus_mean, ivar, _, _, eps = cache
    
    dx_norm = dout * gamma
    dsample_var = -0.5 * np.sum(dx_norm * x_minus_mean, axis=0) * ivar**3
    dsample_mean = -1.0 * np.sum(dx_norm * ivar, axis=0) - 2.0 * dsample_var * np.mean(x_minus_mean, axis=0)
    
    dx = dx_norm * ivar + 2.0/N * dsample_var * x_minus_mean + 1.0/N * dsample_mean
