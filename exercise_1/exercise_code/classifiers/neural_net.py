...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = -np.sum(np.log(softmax[range(N), y] + 1e-12))
        ...
        grads['W2'] = np.dot(H.T, dS) + reg * W2
        grads['b2'] = np.dot(np.ones(N), dS)
    
        dH = np.dot(dS, W2.T)
        dH[H < 1e-12] = 0
        grads['W1'] = np.dot(X.T, dH) + reg * W1
        grads['b1'] = np.dot(np.ones(N), dH)
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    regularization_strengths = [0.05, 0.1, 0.2, 0.5, 1]
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for lr in learning_rates:
                    ...
                    print([bs, hs, reg, lr], val_accuracy)
                    
                    if val_accuracy > best_val:
                        best_val = val_accuracy
                        best_net = net
                        best_params = [bs, hs, reg, lr]
    
    print('best_acc:', best_val)
    print('best batch_size:', best_params[0])
    print('best hidden_size:', best_params[1])
    print('best reg:', best_params[2])
    print('best learning_rate:', best_params[3])
